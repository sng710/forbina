# v363 — Accessibility scaling + mobile logo clearance

- Increased mobile opening quote clearance below the fixed council logo.
- Moved the accessibility toggle/panel to the bottom-right so it cannot cover the desktop left search panel.
- Replaced root-font-size scaling with computed text-element scaling so viewport/clamp typography scales consistently.
- Added reflow safeguards so enlarged text cannot clip or hide the homepage search/filter controls.
- Added dynamic/resize re-scaling for text inserted after load.
- No memorial data, approved order, biography text, images, or links changed.

# v362 — Accessibility toolbar on homepage

- Added the existing self-hosted accessibility toolbar to the homepage.
- Toolbar remains available on all internal memorial pages through `person-bootstrap.js`.
- Added homepage toolbar loading to the release validation gate.
- No memorial data, ordering, biographies, images, links or layout content changed.

# v361 — Mobile logo/text spacing

- Mobile-only visual correction: increased the memorial hero top safe area so the opening quote sits clearly below the fixed Sha’ar HaNegev council logo.
- No content, people order, biography, image, link, filter, or desktop-layout changes.

# Changelog

היסטוריית העבודה נשמרת תחת `docs/archive/`. הקובץ הזה הוא נקודת הכניסה המרכזית במקום עשרות קבצי PATCH/QA ברוט.

## Release 360 — Original homepage order restored

- הוחזר סדר התצוגה המקורי והמאושר של כל 154 המונצחים, כאשר אופיר ליבשטיין ראשון והמשך הרשימה זהה אחד-לאחד לסדר המקור.
- הוסר המיון לפי גיל שנוסף ב-v358/v359 ושינה את סדר הכרטיסים בדף הבית.
- אין שינוי בנתוני ההנצחה, התמונות, הביוגרפיות, הקישורים או דפי האנשים לעומת v359.
- גרסת המטמון הועלתה ל-360 וסונכרנה בין דף הבית, bootstrap וקובץ הגרסה.
- נוסף `tools/expected_home_order.json` וכלי ה-QA חוסם מעתה שינוי לא מכוון בסדר הרשימה או החזרה של מיון לפי גיל.

## Release 359 — Final verified master

- נשמרו במלואם 154 הרשומות וכל השינויים המצטברים של PATCH172.
- הושלמו שישה סיפורי חיים היסטוריים שהיו ריקים, בניסוח תמציתי המבוסס על דפי ההנצחה הרשמיים של משרד הביטחון.
- הושלמה שיוך היישוב `ניר עם` לחיים (קארול) גולדנברג ולעמוס שחר שורץ, ורק במקרים שבהם המקור הרשמי תומך בכך במפורש.
- חיפוש השמות מבוסס מעתה על השם המוצג ללא הסיומות ז״ל/הי״ד, תוך שמירה על הנתון המקורי.
- גרסת המטמון הועלתה ל-359 וסונכרנה בין דף הבית, bootstrap וקובץ הגרסה.
- הורחב `tools/validate_memorial.py`: בדיקת ביוגרפיות, גרסה, sitemap/robots, metadata, תיקיות אישיות, קישורים מקומיים, alt ו-`noopener`.
- נוסף GitHub Actions validator כדי למנוע פריסה של גרסה שנכשלת בבדיקות.
- פריטי מידע היסטוריים שלא ניתן לאמת באופן חד-משמעי נשארו ללא השלמה מנחשת ומתועדים ב-`docs/OPEN_CONTENT_VERIFICATION_ITEMS.md`.

### Latest cumulative patch sources

- [`README_PATCH168.txt`](docs/archive/patch-notes/README_PATCH168.txt)
- [`README_PATCH168_R2_REPAIR.txt`](docs/archive/patch-notes/README_PATCH168_R2_REPAIR.txt)
- [`README_PATCH169.txt`](docs/archive/patch-notes/README_PATCH169.txt)
- [`README_PATCH170.txt`](docs/archive/patch-notes/README_PATCH170.txt)
- [`README_PATCH171.txt`](docs/archive/patch-notes/README_PATCH171.txt)
- [`README_PATCH172.txt`](docs/archive/patch-notes/README_PATCH172.txt)
